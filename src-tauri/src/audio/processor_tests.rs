...
mod tests {
    ...
    fn test_process_and_filter() {
        ...
        assert_eq!(result[0], 0.0); // Below noise threshold
        assert!(result[1] > 0.0); // Amplified
        assert!(result[3] <= 1.0); // Clamped
    }

    #[test]
    fn test_manage_buffer() {
        let mut buffer = vec![0.0; MAX_SAMPLES - 100];
        let window = tauri::Window::default(); // Mock or replace with a test double
    ...
    fn test_calculate_rms() {
        ...
        assert!((rms - 0.707).abs() < 0.01); // Approximation
    }
}